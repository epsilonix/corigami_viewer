...
def main():
    ...
    # New optional argument for normalization method. It currently does nothing.
...
def single_prediction(output_path, celltype, chr_name, start, model_path, seq_path, ctcf_path, atac_path):
    # If celltype is not provided, use an empty string.
    celltype_for_path = celltype if celltype and celltype.strip() != "" else ""
    ...
    # The MatrixPlot constructor will now build a path using celltype_for_path;
    # if celltype_for_path is empty, it will be omitted from the file path.
    plot = plot_utils.MatrixPlot(output_path, pred, 'prediction', celltype_for_path, chr_name, start)
    plot.plot()
